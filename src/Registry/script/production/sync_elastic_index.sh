...
curl -XPUT ${OUTPUT_HOST}/users/_settings -d '{"index":{"blocks.read_only":false}}'
...
#dump users
elasticdump --input=${INPUT_HOST}/users_v1 --output=${OUTPUT_HOST}/users_v1 --type=analyzer
elasticdump --input=${INPUT_HOST}/users_v1 --output=${OUTPUT_HOST}/users_v1 --type=mapping
elasticdump --input=${INPUT_HOST}/users_v1 --output=${OUTPUT_HOST}/users_v1 --type=data
...
curl -XPOST "${OUTPUT_HOST}/_aliases/" -d '{ "actions": [{ "add": { "index": "users_v1", "alias": "users" }} ] }'
